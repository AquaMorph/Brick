...
CinematographyWidget::CinematographyWidget(QWidget* parent) : QWidget(parent) {
  ...
  connect(liveButton_, &QPushButton::clicked, this,
          [this] { showLiveView(); });
  connect(gallery_, &QListWidget::currentRowChanged, this,
          [this](int row) { showTestShot(row); });
  ...
  connect(deleteButton_, &QPushButton::clicked, this,
          [this] { deleteSelectedTestShot(); });

  refreshCameras();
}


void CinematographyWidget::setShot(Project* project, int sceneIndex,
                                    int shotIndex) {
  project_ = project;
...
void CinematographyWidget::selectCamera(int index) {
  ...
  connect(camera_.get(), &CameraSession::settingsChanged, this, [this] {
...
void CinematographyWidget::rebuildSettings() {
  ...
  if (settings_.empty()) {
    settingsLayout_->addRow(new QLabel(camera_ ? "No adjustable settings"
                                              : "Select a camera",
                                            this));
    return;
  }
...
void CinematographyWidget::saveCameraSettings() {
  if (project_ == nullptr || camera_ == nullptr || sceneIndex_ < 0 ||
      shotIndex_ < 0) {
    return;
  }
  QString error;
  if (!project_->saveCurrentShotCameraSettings(
          sceneIndex_, shotIndex_, currentCameraSettings(), &error)) {
    showError(error);
  }
}
...
void CinematographyWidget::restoreShotSettings() {
  ...
  QString error;
  const auto saved =
      project_->currentShotCameraSettings(sceneIndex_, shotIndex_, &error);
  if (!saved.has_value() || saved->backend.isEmpty()) {
    return;
  }
  const auto device = std::find_if(
      devices_.begin(), devices_.end(), [&saved](const CameraDevice& candidate) {
        return candidate.backend == saved->backend &&
               candidate.id == saved->deviceId;
      });
  if (device != devices_.end()) {
    const int comboIndex =
        static_cast<int>(std::distance(devices_.begin(), device)) + 1;
    if (cameraCombo_->currentIndex() != comboIndex) {
      cameraCombo_->setCurrentIndex(comboIndex);
    }
  }
  if (camera_ && camera_->backend() == saved->backend &&
      camera_->deviceId() == saved->deviceId) {
...
void CinematographyWidget::capture() {
  ...
  const QString shotDirectory =
      project_->shotDirectory(sceneIndex_, shotIndex_, &error);
  if (shotDirectory.isEmpty()) {
    showError(error);
    return;
  }
  pendingCapture_ = PendingCapture{
      project_, project_->directory(), shotDirectory, sceneIndex_, shotIndex_,
      camera_->displayName(), currentCameraSettings(),
      currentDisplaySettings()};
  const QString base = captureDirectory_.filePath(
      "capture-" + QString::number(QDateTime::currentMSecsSinceEpoch()));
...
void CinematographyWidget::importCapture(const QString& filePath) {
  ...
  if (project_ != capture.project ||
    ...
    showError("The originating shot changed before capture completed. The "
              "image remains at " + filePath);
    return;
  }
  const auto imported = capture.project->importTestShot(
      capture.sceneIndex, capture.shotIndex, filePath,
      QDateTime::currentDateTimeUtc(), capture.cameraDisplayName,
      capture.cameraSettings, capture.cameraSettings.settings,
...
void CinematographyWidget::refreshGallery(const QString& selectedFileName) {
  ...
  for (int index = 0; index < static_cast<int>(testShots_.size()); ++index) {
    ...
    auto* item = new QListWidgetItem(
        QIcon(thumbnail.scaled(128, 82, Qt::KeepAspectRatio,
  ...
  if (!error.isEmpty()) {
    ...
    statusLabel_->setText(QString("%1 test image%2")
                              .arg(testShots_.size())
                              .arg(testShots_.size() == 1 ? "" : "s"));
  }
}
...
void CinematographyWidget::showTestShot(int row) {
  ...
  QStringList metadata = {
      "<b>" + shot.fileName.toHtmlEscaped() + "</b>",
...
void CinematographyWidget::deleteSelectedTestShot() {
  ...
  QMessageBox confirmation(QMessageBox::Warning, "Delete Test Image",
                           "Delete this test image? This cannot be undone.",
  ...
  QString error;
  if (!project_->deleteTestShot(sceneIndex_, shotIndex_,
                                testShots_[row].fileName, &error)) {
    showError(error);
    return;
  }
  showLiveView();
...
void CinematographyWidget::restoreSelectedTestShotSettings() {
  ...
  const TestShot& shot = testShots_[row];
  if (camera_->backend() != shot.cameraBackend ||
      camera_->deviceId() != shot.cameraDeviceId) {
    const auto device = std::find_if(
        devices_.begin(), devices_.end(), [&shot](const CameraDevice& candidate) {
          return candidate.backend == shot.cameraBackend &&
                 candidate.id == shot.cameraDeviceId;
        });
    if (device == devices_.end()) {
      showError("The camera used for this test image is not connected.");
      return;
    }
    cameraCombo_->setCurrentIndex(
        static_cast<int>(std::distance(devices_.begin(), device)) + 1);
  }
...
void CinematographyWidget::updatePreview(const QImage& image,
